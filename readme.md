...
```
npm install kl-components -D
```

在 vue 项目 main.js 里面引入(目前没有做按需加载 默认全局引入)

```
import KlComponents from "kl-components";
import "kl-components/lib/kl-components.css";
Vue.use(KlComponents);
```
### 下面是具体组件使用方法
#### 1.kl-table（表格组件）
...
kl-table 使用注意事项 1.自定义列内容使用时默认数据 slot-scope="props" 使用方法为 props.scope.row.xx
#### 2.kl-form（表单组件）

```
下面是一个自定义表单的使用例子
  <kl-form @handleEventChange="onHandleEventChange" :formConfig="formConfig">
 </kl-form>
...
export const formConfig = [
         {
           ...
           type: "kl-radio", //  标识属于哪种form组件 kl-radio
           kl-checkbox kl-input kl-select kl-cascader kl-switch
           size: "small", //  大小
...
kl-form 使用注意事项 1.数据源需要后台请求时可以参照 kl-select 配置
